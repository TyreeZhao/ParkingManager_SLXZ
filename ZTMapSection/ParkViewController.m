...
//    [self.mapView addSubview:_addMapviewBtn];
//    [self.mapView addSubview:_subMapviewBtn];
...
//    [self.mapView addSubview:self.roadCondition];
...
//显示路况
//- (IBAction)roadCondition:(id)sender {
...
//    NSInteger index = self.mapViewTypeSegment.selectedSegmentIndex;
//    switch (index) {
//        case 0:
//            _mapView.mapType = BMKMapTypeStandard;
//            break;
...
//        UIAlertView* alert = [[UIAlertView alloc]initWithTitle:@"未知起点" message:@"未知起点，请定位后导航" delegate:nil cancelButtonTitle:@"cancel" otherButtonTitles:nil, nil];
//        [alert show];
        return;
    }
    if (self.endLatitude == 0 || self.endLongitude == 0) {
        [SVProgressHUD showInfoWithStatus:@"未知终点，请搜索停车场后导航"];
//        UIAlertView* alert = [[UIAlertView alloc]initWithTitle:@"未知终点" message:@"未知终点，请搜索停车场后导航" delegate:nil cancelButtonTitle:@"cancel" otherButtonTitles:nil, nil];
//        [alert show];
        return;
    }
